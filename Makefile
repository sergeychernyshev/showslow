...
assets:
	...
	# TODO replace crc32 -v with some hashing tool that exists on all systems and outputs results even for single file
	find ./ -type f | grep -v -E '^./(timeline|timeplot|ajax|users|svn-assets|flot\/examples)/' | grep -E '\.(png|jpg|css|js|gif|ico)$$' | sort | xargs -n10 crc32 -v | sed -e 's/\t\.\//\t/' > asset_versions.tsv
